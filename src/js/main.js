...
require([
    ...
    'use strict';
		...
		function renderSvg(referenceImg, ratio, position){
				...
				return function(){
					...
					var SVG_SIZE_W=450;
					var SVG_SIZE_H=362;
                    var svgRatio = SVG_SIZE_W/svgWidth;
					var height = SVG_SIZE_H/svgRatio;
                    
                    var viewPortSize = getViewPortSize();
                    
                    var viewportWidthRules = (viewPortSize.x > referenceImg.width);
                    if (viewportWidthRules) {
                        var imgWidth = viewPortSize.x;
                        var imgHeight = imgWidth / ratio;
                    } else {
                        var imgHeight = viewPortSize.y;
					...
					var patternWidth = imgWidth * svgRatio;
					var patternHeight = imgHeight * svgRatio;
					var patternTop = - offset.top * svgRatio;

                    switch (position) {
                        case 'left': 
					        var patternLeft = - offset.left * svgRatio;
                            break;
                        case 'right': 
                            var patternLeft = (viewPortSize.x - imgWidth - offset.left) * svgRatio;
                            break;
                        default:
                            var patternLeft = - ( imgWidth/2 - svgWidth/2 ) * svgRatio ;
                            break;
                    }

					svg.outerHTML = (
						'<svg id="ced" viewBox="0 0 '+SVG_SIZE_W+' '+SVG_SIZE_H+'"><pattern id="mypattern" patternUnits="userSpaceOnUse" width="' +
							patternWidth + '" height="' +  patternHeight + '"><svg id="innerced"><image width="'+
							patternWidth +'" height="'+ patternHeight + '" xlink:href="' +
							referenceImg.src + '" x="'+ patternLeft +
    ...
    function resizeAndPositionCanvas(ratio, position) {
        return function() {
            var basicStyle = {
    ...
    imgElm.onload = function() {
        ...
        renderSvg();
        renderSvg();
        window.addEventListener('resize', throttle(renderSvg));
    };
    ...
    var activeGifs = document.querySelectorAll('[data-src]');
    for (var i = 0; i < activeGifs.length; i++) {
        activeGifs[i].addEventListener('mouseover', function() {
            document.body.classList.add("hide-svg");

        });
        activeGifs[i].addEventListener('mouseout', function() {
    ...
    for(var i = 0; i < tooltips.length; i++) {
        tooltips[i].addEventListener('mousemove', function(e) {
            ...
            tooltip.style.top = e.pageY - rect.top + 'px';
        });
    }
