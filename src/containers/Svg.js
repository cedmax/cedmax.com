...
import { throttle, calculateSvgDimensions, isIE } from "../dom-utilities";
...
const calculate = calculateSvgDimensions(constants.SVG_SIZE_W);
const calculateSvgSize = (svg, background) => {
  if (!svg) return {};
...
let resizeSvg;
export default memo(({ background: { ratio, image, alignment }, hide }) => {
  if (isIE) return null;

  const mySvg = useRef(null);
  const [
    { patternHeight, patternWidth, patternLeft, patternTop },
    setSvgSize,
  ...
  useEffect(() => {
    if (typeof document !== "undefined") {
      ...
      resizeSvg = () =>
        setSvgSize(calculateSvgSize(mySvg.current, { ratio, alignment }));
  ...
  return (
    <svg
      ...
      id="ced"
      ref={mySvg}
      viewBox={`0 0 ${constants.SVG_SIZE_W} ${constants.SVG_SIZE_H}`}
    >
      {patternWidth && (
        <Fragment>
          <pattern
