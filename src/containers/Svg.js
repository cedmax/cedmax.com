...
import { calculateSvgDimensions, isIE } from "../dom-utilities";
...
const calculateSvgSize = (svg, header, background) => {
  if (!svg) return {};
...
let resizeSvg;
export default memo(
  ({ headerRef, background: { ratio, image, alignment }, hide }) => {
    if (isIE) return null;
    ...
    useEffect(() => {
      if (typeof document !== "undefined") {
        ...
        resizeSvg = () =>
          setSvgSize(
            calculateSvgSize(mySvg.current, headerRef.current, {
              ratio,
              alignment,
            })
          );
